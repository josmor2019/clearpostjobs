...
export default function CoverLetterPage() {
  ...
  async function handleGenerate(e: FormEvent) {
    ...
    const res = await fetch("/api/ai/cover-letter", {
      ...
      body: JSON.stringify({ name, email, jobTitle, company, jobDescription, userBackground: background, tone }),
    });
      ...
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-neutral-200 bg-white p-5 lg:block">
        ...
        <nav className="space-y-1">
          {[
            ...
            { href: "/dashboard/resume", label: "AI Resume Builder" },
            { href: "/dashboard/cover-letter", label: "AI Cover Letter", active: true },
            { href: "/jobs", label: "Browse Jobs" },
          ].map(({ href, label, active }) => (
            <a key={href} href={href} className={`inline-flex w-full items-center rounded-lg px-3 py-2 text-sm font-medium transition-colors ${active ? "bg-[#1D9E75]/10 text-[#188a66]" : "text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900"}`}>
              {label}
            </a>
          ))}
        </nav>
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 border-b border-neutral-200 bg-white/95 backdrop-blur">
          <div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <h1 className="text-lg font-semibold text-neutral-900">AI Cover Letter Builder</h1>
            <a href="/dashboard/resume" className="rounded-lg border border-neutral-200 bg-white px-3 py-1.5 text-sm font-semibold text-neutral-700 hover:bg-neutral-50">
              ← Resume
            </a>
          </div>
        </header>
          ...
          <div className="grid gap-8 xl:grid-cols-2">
            <form onSubmit={(e) => void handleGenerate(e)} className="space-y-4 rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
              <div>
                ...
                <p className="mt-1 text-sm text-neutral-500">Paste the job description for the best results.</p>
              </div>
              ...
              <div>
                <label htmlFor="cl-jd" className="mb-1.5 block text-sm font-medium text-neutral-700">Job description</label>
                <textarea id="cl-jd" value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste the full job description here for a tailored letter." className={`${inputClass} min-h-[140px] resize-y`} rows={5} />
              </div>

              <div>
                <label htmlFor="cl-bg" className="mb-1.5 block text-sm font-medium text-neutral-700">Your background</label>
                <textarea id="cl-bg" value={background} onChange={(e) => setBackground(e.target.value)} placeholder="Briefly describe your relevant experience and why you're excited about this role." className={`${inputClass} min-h-[100px] resize-y`} rows={3} />
              </div>
                  ...
                  {TONES.map((t) => (
                    <button
                      ...
                      className={`rounded-xl border px-3 py-2 text-left transition-colors ${
                        tone === t.value
                          ? "border-[#1D9E75] bg-[#1D9E75]/8 text-[#188a66]"
              ...
              {letter ? (
                <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    <h2 className="text-base font-semibold text-neutral-900">Your Cover Letter</h2>
                    <div className="flex gap-2">
                      <button onClick={() => void navigator.clipboard.writeText(letter)} className="rounded-lg border border-neutral-200 px-3 py-1.5 text-xs font-semibold text-neutral-700 hover:bg-neutral-50">
                        Copy
                      </button>
                      <button onClick={() => void handleDetect()} disabled={detecting} className="rounded-lg bg-[#1D9E75]/10 px-3 py-1.5 text-xs font-semibold text-[#188a66] hover:bg-[#1D9E75]/20 disabled:opacity-60">
                  ...
                  {detect ? (
                    <div className="mt-4 rounded-xl border border-neutral-200 bg-neutral-50 p-4">
                      <div className="mb-2 flex items-center justify-between">
                        ...
                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${(detect.aiScore ?? 0) < 30 ? "bg-emerald-100 text-emerald-700" : (detect.aiScore ?? 0) < 60 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}>
                          {detect.aiScore ?? 0}% AI — {detect.verdict}
                        </span>
                      </div>
                      {detect.recommendation ? <p className="text-xs font-medium text-[#188a66]">{detect.recommendation}</p> : null}
                    </div>
                  ) : null}
                </div>
              ) : (
                <div className="flex h-64 items-center justify-center rounded-2xl border-2 border-dashed border-neutral-200 bg-white">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium text-neutral-700">Cover letter will appear here</p>
                    <p className="mt-1 text-xs text-neutral-500">Fill in job details and click Generate</p>
                  </div>
                </div>
              )}
            </div>
